...
def check_city(inp, exp):
    if inp.name != exp.name:
      print(f'ERROR ON NAME inp: {inp.name} expected : {exp.name}')
      return False
    if inp.lat != exp.lat:
      print(f'ERROR ON LAT inp: {inp.lat} expected : {exp.lat}')
      return False
    if inp.lon != exp.lon:
      print(f'ERROR ON LON inp: {inp.lon} expected : {exp.lon}')
      return False
    return True
